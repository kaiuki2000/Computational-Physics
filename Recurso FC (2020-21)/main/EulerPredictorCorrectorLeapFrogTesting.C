...
int main()
{
    ...
    //Euler
    vector<ODEPoint> vsol = solver.Euler(P0, h, T);

    std::vector<double> T1;
    std::vector<double> X1;

    cout << "EulerPoints:" << endl;
    for (auto i : vsol)
    {
        cout << i << endl;
    ...
    cout << "PredictorCorrectorPoints:" << endl;
    for (auto i : vsol)
    {
        cout << i << endl;
        T1.push_back(i.GetTime());
    ...
    G2->SetTitle("PredictorCorrector");
    ...
    C1->BuildLegend();
    C1->Modified();
    C1->SaveAs("Euler&PredictorCorrector&LeapFrogTesting.pdf", "A");
