...
int main()
{
    ...
    TFormula ArgFormula1("Formula1", "-9.81*sin(x[1])-0.3*x[2]");

    ArgFormulas.push_back(ArgFormula0);
    ArgFormulas.push_back(ArgFormula1);

    ODESolver ODE1(ArgFormulas);
    std::vector<double> Point0{0., 70 * TMath::Pi() / 180., 0.};
    ODEPoint ODE1Point0(2, Point0);

    std::vector<ODEPoint> SolPoints2 = ODE1.LeapFrogSolver(ODE1Point0, 0.01, 10000);
    std::vector<ODEPoint> SolPoints3 = ODE1.RK4Solver(ODE1Point0, 0.01, 10000);
    ...
    for (auto i : SolPoints3)
    {
        ...
        E3.push_back(0.5 * 1 * i.GetX(1) * i.GetX(1) + 1 * 9.81 * (1 - cos(i.GetX(0))));
        A3.push_back(sqrt(i.GetX(1) * i.GetX(1) + 2 * 9.81 * (1 - cos(i.GetX(0)))));
    }
    std::cout << std::endl;
    ...
    TGraph *G3 = new TGraph(T3.size(), &T3[0], &A3[0]);
