...
std::vector<ODEPoint> ODESolver::RK2Solver(const ODEPoint &Point0, double h, int IterationsN)
{
    std::vector<ODEPoint> SolutionPoints;
    SolutionPoints.push_back(Point0);
    ...
    for (int i = 0; i < IterationsN; i++)
    {
        ...
        for (int j = Formulas.size(); j > 0; j--)
        {
            TempPoint1[j] = CurrentPoint[j] + (h / 2.) * Formulas[j - 1].EvalPar(&(CurrentPoint.GetVector()[0]));
            NextPoint[j] = CurrentPoint[j] + h * Formulas[j - 1].EvalPar(&(TempPoint1.GetVector()[0]));
        }

        SolutionPoints.push_back(NextPoint);
        CurrentPoint = NextPoint;
        TempPoint1 = NextPoint;
    }
...
std::vector<ODEPoint> ODESolver::RK4Solver(const ODEPoint &Point0, double h, int IterationsN)
{
    ...
    ODEPoint K1Point(Point0);
    ODEPoint K2Point(Point0);
    ODEPoint K3Point(Point0);
    ODEPoint K4Point(Point0);
    ...
    for (int i = 0; i < IterationsN; i++)
    {
        ...
        for (int j = Formulas.size(); j > 0; j--)
        {
            K2Point[j] = CurrentPoint[j] + (h / 2.) * Formulas[j - 1].EvalPar(&(K1Point.GetVector()[0]));

            K3Point[j] = CurrentPoint[j] + (h / 2.) * Formulas[j - 1].EvalPar(&(K2Point.GetVector()[0]));

            K4Point[j] = CurrentPoint[j] + h * Formulas[j - 1].EvalPar(&(K3Point.GetVector()[0]));

            NextPoint[j] = CurrentPoint[j] + (h / 6.) * (Formulas[j - 1].EvalPar(&(K1Point.GetVector()[0])) +
                                                         2. * Formulas[j - 1].EvalPar(&(K2Point.GetVector()[0])) +
                                                         2. * Formulas[j - 1].EvalPar(&(K3Point.GetVector()[0])) +
                                                         Formulas[j - 1].EvalPar(&(K4Point.GetVector()[0])));
        }
...
//4th order Runge-Kutta method: This 1st try was unsuccessful
/*std::vector<ODEPoint> ODESolver::RK4Solver(const ODEPoint &Point0, double h, int IterationsN)
{
    std::vector<ODEPoint> SolutionPoints;
    SolutionPoints.push_back(Point0);
...
//4th order Runge-Kutta method: This 2nd try was unsuccessful
/*std::vector<ODEPoint> ODESolver::RK4Solver(const ODEPoint &Point0, double h, int IterationsN)
{
    std::vector<ODEPoint> SolutionPoints;
    SolutionPoints.push_back(Point0);
...
std::vector<ODEPoint> ODESolver::ShootingMethodBVPSolver1(const ODEPoint &Point, double x1, double x1Image, int x1DerivativeBool, int RK4IterationN)
{
    auto F = [&Point, &x1, &x1Image, &x1DerivativeBool, &RK4IterationN, this](double *u, double *par) {
        ...
        std::vector<ODEPoint> RK4Points = RK4Solver(Point0, h, RK4IterationN);
    ...
    return RK4Solver(TruePoint0, h, RK4IterationN);
}
