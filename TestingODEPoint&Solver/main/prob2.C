...
int main()
{
    ...
    int n = 4;
    double rArray[n] = {5.75, 12.00, 18.00, 30.00};
    double cArray[n] = {1.2e-3, 1.8e-3, 2.6e-3, 4.0e-3};

    //Using NewtonInterpolator...
    NewtonInterpolator NI_Data1(n, rArray, cArray);
    NI_Data1.Print("Non-existant");
    NI_Data1.Draw();

    const double r = NI_Data1.Interpolate(15.);

    std::cout << "\nEstimated r (calculated using Newton interpolation): r= " << r << std::endl;

    //Solving differential equation with Runge-Kutta 4th order
    std::vector<TFormula> ArgFormulas;
    TFormula ArgFormula0("Formula0", "x[2]");
    TFormula ArgFormula1("Formula1", "(-9.8/5.)*sin(x[1])-0.00218621*x[2]"); //With Newton interpolation, I obtained r=0.00218621.
    ...
    //Omega(Time) graph
    TGraph *G3 = new TGraph(T1.size(), &T1[0], &Omega1[0]);
    ...
    C2->BuildLegend();
    C2->Modified();
    C2->SaveAs("prob2_diagramafase.pdf", "AP");
    ...
    Mg3->GetYaxis()->SetTitle("#omega (degree s^{-1})");

    C3->BuildLegend();
    C3->Modified();
    C3->SaveAs("prob2_energia.pdf", "AP");
