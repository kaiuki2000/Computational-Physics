...
//Constructor 1' (Created specifically for Romberg integration. Most likely unusable for any other situation.)
Vec::Vec(int n, double val, char c) : N(n)
{
#ifdef DEBUGBUILD
    printf("[%s]\n", __PRETTY_FUNCTION__);
#endif
...
Vec::~Vec()
{
    ...
    printf("[%s]\n", __PRETTY_FUNCTION__);
#endif
    if (entries) // if(entries) was insufficient? -> if (entries && N != 0) ?? -> removed!
        delete[] entries;
}
...
double Vec::maxAbs()
{
    ...
    printf("[%s]\n", __PRETTY_FUNCTION__);
#endif
    return *std::max_element(entries, entries + N, [](double x1, double x2) { return fabs(x1) < fabs(x2); });
}
